...
full_df.to_csv(OUTPUT_DIRECTORY + f"/NCSU_{STATE}_Unedited.csv")

#Cleaning
data = full_df.dropna(axis=1,how="all")
data.columns = [x.replace(":","") for x in data.columns]
data.index = [" ".join(x.split("-")).capitalize() for x in data.index]


data.to_csv(OUTPUT_DIRECTORY + "/NCSU_{STATE}_Formatted.csv")
